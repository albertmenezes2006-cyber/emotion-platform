"""Plugin: ensemble_emocional | datascience | Ensemble de modelos emocionais com votação"""
...
class EnsembleEmocionalPlugin(PluginBase):
    name = "ensemble_emocional"; version = "1.0.0"
...
@router.get("/status")
async def status():
    return {"plugin": "ensemble_emocional", "categoria": "datascience", "total": len(_db), "ts": datetime.utcnow().isoformat()}
...
plugin = EnsembleEmocionalPlugin()

...
Plugin: certificados
Categoria: educacao
Descrição: Emissão de certificados de conclusão de cursos
"""
...
router = APIRouter(prefix="/api/v1/certificados", tags=["educacao"])
_db = {}

class CertificadosPlugin(PluginBase):
    name = "certificados"
    version = "1.0.0"
...
@router.get("/status")
async def status():
    return {"plugin": "certificados", "categoria": "educacao", "total": len(_db), "ts": datetime.utcnow().isoformat()}

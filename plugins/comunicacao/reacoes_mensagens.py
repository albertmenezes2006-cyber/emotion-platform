"""
Plugin: reacoes_mensagens
Categoria: comunicacao
Descrição: Reações emocionais terapêuticas a mensagens
"""
...
router = APIRouter(prefix="/api/v1/reacoes-mensagens", tags=["comunicacao"])
_db = {}

class ReacoesMensagensPlugin(PluginBase):
    name = "reacoes_mensagens"
    version = "1.0.0"
    description = "Reações emocionais terapêuticas a mensagens"
...
@router.get("/status")
async def status():
    return {"plugin": "reacoes_mensagens", "categoria": "comunicacao", "total": len(_db), "ts": datetime.utcnow().isoformat()}

...
Descrição: Notas de voz para comunicação emocional assíncrona
"""
...
class VoiceNotesPlugin(PluginBase):
    name = "voice_notes"
    version = "1.0.0"
    description = "Notas de voz para comunicação emocional assíncrona"
...
@router.get("/status")
async def status():
    return {"plugin": "voice_notes", "categoria": "comunicacao", "total": len(_db), "ts": datetime.utcnow().isoformat()}

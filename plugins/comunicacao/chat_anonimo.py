...
Descrição: Chat anônimo para suporte emocional entre usuários
"""
...
class ChatAnonimoPlugin(PluginBase):
    name = "chat_anonimo"
    version = "1.0.0"
    description = "Chat anônimo para suporte emocional entre usuários"
...
@router.get("/status")
async def status():
    return {"plugin": "chat_anonimo", "categoria": "comunicacao", "total": len(_db), "ts": datetime.utcnow().isoformat()}

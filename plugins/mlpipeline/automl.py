...
Descrição: AutoML para seleção automática de algoritmos
"""
...
class AutomlPlugin(PluginBase):
    name = "automl"
    version = "1.0.0"
    description = "AutoML para seleção automática de algoritmos"
    category = "mlpipeline"
...
@router.get("/status")
async def status():
    return {"plugin": "automl", "categoria": "mlpipeline", "total": len(_db), "ts": datetime.utcnow().isoformat()}

"""
Plugin: pipeline_treinamento
Categoria: mlpipeline
Descrição: Pipeline completo de treinamento de modelos ML
"""
...
router = APIRouter(prefix="/api/v1/pipeline-treinamento", tags=["mlpipeline"])
_db = {}

class PipelineTreinamentoPlugin(PluginBase):
    name = "pipeline_treinamento"
    version = "1.0.0"
    description = "Pipeline completo de treinamento de modelos ML"
    category = "mlpipeline"
...
@router.get("/status")
async def status():
    return {"plugin": "pipeline_treinamento", "categoria": "mlpipeline", "total": len(_db), "ts": datetime.utcnow().isoformat()}

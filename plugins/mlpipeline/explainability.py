...
Descrição: Explicabilidade SHAP/LIME para modelos em produção
"""
...
class ExplainabilityPlugin(PluginBase):
    name = "explainability"
    version = "1.0.0"
    description = "Explicabilidade SHAP/LIME para modelos em produção"
    category = "mlpipeline"
...
@router.get("/status")
async def status():
    return {"plugin": "explainability", "categoria": "mlpipeline", "total": len(_db), "ts": datetime.utcnow().isoformat()}

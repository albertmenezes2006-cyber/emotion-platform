"""
Plugin: drift_detection
Categoria: mlpipeline
Descrição: Detecção de data drift e concept drift em produção
"""
...
router = APIRouter(prefix="/api/v1/drift-detection", tags=["mlpipeline"])
_db = {}

class DriftDetectionPlugin(PluginBase):
    name = "drift_detection"
    version = "1.0.0"
    description = "Detecção de data drift e concept drift em produção"
    category = "mlpipeline"
...
@router.get("/status")
async def status():
    return {"plugin": "drift_detection", "categoria": "mlpipeline", "total": len(_db), "ts": datetime.utcnow().isoformat()}

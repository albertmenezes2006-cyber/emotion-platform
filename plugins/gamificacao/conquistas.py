...
Descrição: Sistema de conquistas e badges por metas terapêuticas
"""
...
router = APIRouter(prefix="/api/v1/conquistas", tags=["gamificacao"])
_db = {}

class ConquistasPlugin(PluginBase):
    name = "conquistas"
    version = "1.0.0"
    description = "Sistema de conquistas e badges por metas terapêuticas"
...
@router.get("/status")
async def status():
    return {"plugin": "conquistas", "categoria": "gamificacao", "total": len(_db), "ts": datetime.utcnow().isoformat()}

...
Descrição: Sistema de recompensas por engajamento e progresso
"""
...
class RecompensasPlugin(PluginBase):
    name = "recompensas"
    version = "1.0.0"
    description = "Sistema de recompensas por engajamento e progresso"
...
@router.get("/status")
async def status():
    return {"plugin": "recompensas", "categoria": "gamificacao", "total": len(_db), "ts": datetime.utcnow().isoformat()}

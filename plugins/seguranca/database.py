"""Plugin: database | seguranca | Segurança de banco de dados"""
...
class DatabasePlugin(PluginBase):
    name = "database"
    version = "1.0.0"
    description = "Segurança de banco de dados"
    category = "seguranca"
...
@router.get("/status")
async def status():
    return {"plugin": "database", "categoria": "seguranca", "ts": datetime.utcnow().isoformat()}

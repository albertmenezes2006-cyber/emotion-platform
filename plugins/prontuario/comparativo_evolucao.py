"""Plugin: comparativo_evolucao | prontuario | Comparativo de evolução por período"""
...
class ComparativoEvolucaoPlugin(PluginBase):
    name = "comparativo_evolucao"; version = "1.0.0"
...
@router.get("/status")
async def status():
    return {"plugin": "comparativo_evolucao", "categoria": "prontuario", "total": len(_db), "ts": datetime.utcnow().isoformat()}
...
plugin = ComparativoEvolucaoPlugin()
